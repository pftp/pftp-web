...
# You can talk to Winston using the 'raw_input' statement! Since Winston lives in Python,
# your message to Winston needs to be saved into a variable:
...
# Notice that you just wrote a couple of lines of code that will have a different behavior
# (what Winston says back to you) depending on the input (the food you give it)!
# This is a powerful way to handle every possible input that you could be given!

# Here's an idea: since we can input information into a variable using 'raw_input',
# and we can control the size of shapes using a variable, why don't we combine them?
# That means you can tell Winston what size square to draw and he'll do it!

# Now for a quick detour on data types - we mentioned earlier that variables store values,
# but one thing that we didn't mention was that there are different types of values!
...
# One thing to note is that mathematical operations like +,-,*, and / try to give back values
# which are the same type as the inputs.
# For example
3/4
# in python will show
0
# because when the '/' operation is given integers as input, it will return an integer.
# This means that it will "truncate" the decimal
0.75
# and just return the integer part
0

# If you wanted Python to not truncate, you would perform the division with one of the numbers
# as a float. Try out the following:
print 6/7
print 6.0/7
print 6/7.0
print 6.0/7.0
# As you can see, these are all the same division, but Python interprets them differently
# based on the type of the data. When you are doing arithmetic where one input is an integer
# and the other is a float like
6/7.0
# floats take priority over integers and '/' will return a float value instead of an integer.
...
# Note that the last line is problematic because strings can contain any character!
# Try running
int('THIS STRING DOES NOT HAVE ANY NUMBERS IN IT')
# You should see that Python gives you an error message that basically says that it doesnt
# know how to convert it into an integer.
...
# First, note that 'raw_input' will give you back a string type, so we need to convert it
# to an integer before Winston can move forward!
...
# Now here's the cool part - these also work for strings! Strings are compared alphabetically
# in this case.
...
# if statments let you run code only under certain conditions (the 'if' part of the if statement).
# If statements look like this:
if variable:
  print 'variable is True'
# where variable is anything that evaluates to a boolean (True or False).
...
# and you can have multiple 'elif' portions, if you have more than 2 scenarios.
price = 1.50
if money < price:
...
# Now you can use if statements to run different code, i.e. draw different shapes based on a condition:
# Here's a short example that does something similar:
...
# Now its your turn - use a raw_input so Winston can ask what shape to draw,
# and another raw_input so Winston can ask what size shape to draw
# and then write an if statement to draw a triangle or square with the inputted size.
# if the shape isn't recognized, then print your own error message like above.

# Great work! Do you feel the power yet? You have just increased your programming
# vocabulary to include Variables, Input, Output, and If statements.
# These are fundamental building blocks for programming, and we will explore some more
# next week!
